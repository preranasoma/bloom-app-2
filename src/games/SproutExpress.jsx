...
class GameScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.spritesheet('plants', '/assets/tilesets/Farming Plants v2 watered.png', { frameWidth: 16, frameHeight: 16 });
    this.load.spritesheet('player', '/assets/player.png', { frameWidth: 48, frameHeight: 48 });
    this.load.spritesheet('playerAction', '/assets/Basic Charakter Actions.png', { frameWidth: 48, frameHeight: 48 });
    this.load.image('dialogBox', '/assets/tilesets/dialog box.png');
    this.load.audio('bing', '/assets/Audio/bing_1.wav');
  }

  loadFarmSave() {
    try { const raw = localStorage.getItem('farmSave'); return raw ? JSON.parse(raw) : {}; } catch { return {}; }
  }
  savePlot(key, plotData) {
    try { const save = this.loadFarmSave(); save[key] = { plantType: plotData.plantType, stage: plotData.stage, watered: plotData.watered }; localStorage.setItem('farmSave', JSON.stringify(save)); } catch {}
  }
  deletePlotSave(key) {
    try { const save = this.loadFarmSave(); delete save[key]; localStorage.setItem('farmSave', JSON.stringify(save)); } catch {}
  }
  loadInventorySave() {
    try { const raw = localStorage.getItem('inventorySave'); const saved = raw ? JSON.parse(raw) : null; const result = Array(INVENTORY_SIZE).fill(null); if (Array.isArray(saved)) saved.slice(0, INVENTORY_SIZE).forEach((item, i) => { result[i] = item; }); return result; } catch { return Array(INVENTORY_SIZE).fill(null); }
  }
  saveInventory() {
    try { localStorage.setItem('inventorySave', JSON.stringify(this.inventoryItems)); } catch {}
  }
  ...
  create() {
    ...
    waterLayer.setPosition(-(map.widthInPixels * 0.1) / 2, -(map.heightInPixels * 0.1) / 2);
    this.tweens.add({ targets: waterLayer, x: { from: -(map.widthInPixels * 0.1) / 2 - 2, to: -(map.widthInPixels * 0.1) / 2 + 2 }, duration: 1500, yoyo: true, repeat: -1, ease: 'Sine.inout' });
    ...
    this.farmData = {};
    this.tileSize = 16;
    this.plantTypes = Array.from({ length: 33 }, (_, row) => ({ id: row, stages: Array.from({ length: 6 }, (_, s) => row * 7 + s) }));
    ...
    Object.entries(savedFarm).forEach(([key, saved]) => {
      ...
      if (ptId === 1) ptId = 19;
      if (!VALID_PLANT_TYPES.includes(ptId)) ptId = VALID_PLANT_TYPES[ptId % VALID_PLANT_TYPES.length];
      if (restoredTypes.has(ptId)) ptId = VALID_PLANT_TYPES.find(t => !restoredTypes.has(t)) ?? ptId;
      ...
      const plant = this.add.sprite(tileX * this.tileSize + 8, tileY * this.tileSize + 8, 'plants', plantType.stages[stage]);
    ...
    const farmTiles = [];
    this.farmLayer.forEachTile(tile => { if (tile && tile.index !== -1) farmTiles.push({ x: tile.x, y: tile.y }); });
    farmTiles.forEach(({ x: tileX, y: tileY }) => {
      const key = `${tileX},${tileY}`;
      if (this.farmData[key]) return;
      const assignedType = VALID_PLANT_TYPES.find(t => !usedTypes.has(t)) ?? VALID_PLANT_TYPES[Object.keys(this.farmData).length % VALID_PLANT_TYPES.length];
      usedTypes.add(assignedType);
      const plantType = this.plantTypes[assignedType];
      const maxStage = plantType.stages.length - 1;
      this.farmLayer.removeTileAt(tileX, tileY);
      const plant = this.add.sprite(tileX * this.tileSize + 8, tileY * this.tileSize + 8, 'plants', plantType.stages[maxStage]);
      this.plants.add(plant);
      this.farmData[key] = { sprite: plant, stage: maxStage, plantType: assignedType, watered: false };
    });

    this.housePositions = [
      { x: 6.5, y: 2.5 }, { x: 24.5, y: 2.5 }, { x: 2.5, y: 6.5 },
      { x: 26.5, y: 8.5 }, { x: 9.5, y: 15.5 }, { x: 19.5, y: 14.5 },
    ];
    ...
    this.inventoryBar = this.add.image(0, 0, 'inventoryBar').setScrollFactor(0).setOrigin(1, 1).setDepth(1000);
    this.inventoryIcons = this.inventoryItems.map(() => this.add.sprite(0, 0, 'plants', 0).setScrollFactor(0).setDepth(1002).setOrigin(0.5).setVisible(false).setScale(1.5));
    this.inventorySelection = this.add.rectangle(0, 0, 40, 40).setStrokeStyle(2, 0xffff00).setScrollFactor(0).setDepth(999).setOrigin(0.5);
    ...
    this.scale.on('resize', gameSize => { this.uiCamera.setSize(gameSize.width, gameSize.height); this.positionInventoryBar(); });
    ...
    this.spawnEvent = this.time.addEvent({ delay: this.spawnDelay, callback: this.trySpawnRequest, callbackScope: this, loop: true });
  }

  positionInventoryBar() {
    const { width, height } = this.scale;
    const padding = 16, barX = width - padding, barY = height - padding;
    const slotSpacing = 48, barLeft = barX - this.inventoryBar.displayWidth;
    const startX = barLeft + 30, slotY = barY - 43;
  ...
  updateInventoryDisplay() {
    this.inventoryItems.forEach((item, i) => {
      ...
      if (item !== null) icon.setFrame(item.frame).setVisible(true);
  ...
  trySpawnRequest() {
    ...
    request.cropType = cropType; request.active = true; request.deadline = this.time.now + this.requestTimeLimit;
    this.showRequestBubble(request);
  }

  showRequestBubble(request) {
    const worldX = request.house.x * this.tileSize + 42, worldY = request.house.y * this.tileSize + 15;
  ...
  triggerGameOver() {
    ...
    const overlay = this.add.rectangle(cx, cy, width, height, 0x000000, 0.72).setScrollFactor(0).setDepth(900);
    const title = this.add.text(cx, cy - 40, 'GAME OVER', { font: 'bold 36px Arial', fill: '#ff4444', stroke: '#000000', strokeThickness: 5 }).setScrollFactor(0).setDepth(901).setOrigin(0.5);
    const score = this.add.text(cx, cy + 5, `Deliveries completed: ${this.completedDeliveries}`, { font: '18px Arial', fill: '#ffffff', stroke: '#000000', strokeThickness: 3 }).setScrollFactor(0).setDepth(901).setOrigin(0.5);
    const hint = this.add.text(cx, cy + 38, 'Press R to play again', { font: '13px Arial', fill: '#cccccc', stroke: '#000000', strokeThickness: 2 }).setScrollFactor(0).setDepth(901).setOrigin(0.5);
  ...
  checkDelivery() {
    if (this.gameOver) return;
    const px = this.player.x / this.tileSize, py = this.player.y / this.tileSize;
    const request = this.deliveryRequests.find(r => r.active && Math.abs(r.house.x - px) <= 2 && Math.abs(r.house.y - py) <= 2);
    if (!request) return;
    const slotIndex = this.inventoryItems.findIndex(item => item !== null && item.plantType === request.cropType);
  ...
  update() {
    ...
    if (Phaser.Input.Keyboard.JustDown(this.keyE) && !this.isActing) {
      ...
      let tileX = Math.floor(player.x / this.tileSize), tileY = Math.floor(player.y / this.tileSize);
      ...
      if (tool === 'tool1') {
        if (this.farmData[key]) {
          const crop = this.farmData[key], plantType = this.plantTypes[crop.plantType], maxStage = plantType.stages.length - 1;
          if (crop.stage >= maxStage) {
            const added = this.addToInventory({ plantType: crop.plantType, frame: plantType.stages[maxStage] });
            if (!added) { this.showFloatingText(tileX * this.tileSize + 8, tileY * this.tileSize, 'Inventory Full!', '#ff4444'); player.once('animationcomplete', () => { this.isActing = false; }); return; }
            ...
            crop.stage = 0; crop.watered = false; crop.sprite.setFrame(plantType.stages[0]); this.savePlot(key, crop);
          }
        } else if (tile && !this.farmData[key]) {
          const savedFarm = this.loadFarmSave(), usedTypes = new Set(Object.values(this.farmData).map(c => c.plantType));
          let assignedType;
          const prevSaved = savedFarm[key];
          if (prevSaved !== undefined && VALID_PLANT_TYPES.includes(prevSaved.plantType) && !usedTypes.has(prevSaved.plantType)) assignedType = prevSaved.plantType;
          else assignedType = VALID_PLANT_TYPES.find(t => !usedTypes.has(t)) ?? VALID_PLANT_TYPES[Object.keys(this.farmData).length % VALID_PLANT_TYPES.length];
          this.farmLayer.removeTileAt(tileX, tileY);
          const plant = this.add.sprite(tileX * this.tileSize + 8, tileY * this.tileSize + 8, 'plants', this.plantTypes[assignedType].stages[0]);
          this.uiCamera.ignore(plant);
          this.plants.add(plant);
          const cropData = { sprite: plant, stage: 0, plantType: assignedType, watered: false };
          this.farmData[key] = cropData; this.savePlot(key, cropData);
          this.trySpawnRequest();
        }
      }
      ...
      if (tool === 'tool2') {
        if (this.farmData[key]) {
          ...
          if (crop.stage < maxStage) { crop.stage++; crop.watered = true; crop.sprite.setFrame(plantType.stages[crop.stage]); this.savePlot(key, crop); }
        }
      }
    ...
    if (Phaser.Input.Keyboard.JustDown(this.keySpace) || Phaser.Input.Keyboard.JustDown(this.keyEnter)) this.checkDelivery();
    if (this.isActing) return;

    player.setVelocity(0);
    if (cursors.left.isDown || wasd.left.isDown) { player.setVelocityX(-speed); player.anims.play('walk-left', true); this.lastDirection = 'left'; }
    else if (cursors.right.isDown || wasd.right.isDown) { player.setVelocityX(speed); player.anims.play('walk-right', true); this.lastDirection = 'right'; }
    else if (cursors.up.isDown || wasd.up.isDown) { player.setVelocityY(-speed); player.anims.play('walk-up', true); this.lastDirection = 'up'; }
    else if (cursors.down.isDown || wasd.down.isDown) { player.setVelocityY(speed); player.anims.play('walk-down', true); this.lastDirection = 'down'; }
    else player.anims.play(`idle-${this.lastDirection}`, true);
  }

  showFloatingText(x, y, message, color = '#ffffff') {
    const text = this.add.text(x, y, message, { font: '8px Arial', fill: color, stroke: '#000000', strokeThickness: 2 }).setDepth(200).setOrigin(0.5);
